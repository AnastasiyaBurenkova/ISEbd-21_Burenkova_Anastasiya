...
namespace WindowsFormsApplication18
{
    class Parking
    {
        ...
        {
            parkingStages = new List<ClassArray<ITransport>>(countPlaces);
                    ...
                    foreach (var level in parkingStages)
                    {
                        ...
                        for (int i = 0; i < countPlaces; i++)
                        {
                            ...
                            if (car != null)
                            {
                                ...
                                {
                                    info = new UTF8Encoding(true).GetBytes("Gruzovik:");
                                    fs.Write(info, 0, info.Length);
                                }
        ...
        public bool LoadData(string filename)
        {
            ...
            using (FileStream fs = new FileStream(filename, FileMode.Open))
            {
                ...
                for (int i = 1; i < strs.Length; i++)
                {
                    ...
                    else if (strs[i].Split(':')[0] == "Gruzovik")
                    {
                        ITransport car = new Gruzovik(strs[i].Split(':')[1]);
                        int number = parkingStages[counter] + car;
                        if (number == -1)
                        {
                            return false;
