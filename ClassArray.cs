...
namespace WindowsFormsApplication18
{
    class ClassArray<T> : IEnumerator<T>, IEnumerable<T>, IComparable<ClassArray<T>> where T : ITransport
    {
        ...
        {
            var isGruzovik = car is Gruzovik;
            ...
            for (int i = 0; i < p.places.Count; i++)
            {
                ...
                if (car.GetType() == p.places[i].GetType())
                {
                    if (isGruzovik)
                    {
                        if ((car as Gruzovik).Equals(p.places[i]))
                        {
            ...
            else
            {
                ...
                for (int i = 0; i < this.places.Count; ++i)
                {
                    if (this.places[thisKeys[i]] is Car && 
                        other.places[thisKeys[i]] is Gruzovik)
                    {
                        return 1;
                    }
                    if (this.places[thisKeys[i]] is Gruzovik &&
                    ...
                    if (this.places[thisKeys[i]] is Gruzovik &&
                        other.places[thisKeys[i]] is Gruzovik)
                    {
                        return (this.places[thisKeys[i]] as Gruzovik).CompareTo(other.places[otherKeys[i]] as Gruzovik);
                    }
